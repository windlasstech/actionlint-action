...
- Docker-based reusable GitHub Action wrapping `rhysd/actionlint` with explicit inputs (`paths`, `config-file`, `ignore`, `shellcheck`, `pyflakes`, `format`, `no-color`, `oneline`).
...
- Self-test workflow exercising valid, invalid, config-file, ignore, tool-disable, and formatting inputs.
